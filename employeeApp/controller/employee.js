...
const cron = require('../functions/scheduler');
...
exports.registerUser = (req, res) => {
    ...
    crud.selectOrDeleteQuery(checkquery).then(result => {
        if (result.data.length > 0) res.render('login');
        else {
...
exports.profile = (req, res) => {
    ...
    crud.selectOrDeleteQuery(checkquery).then(resultdata => {
        res.render('profile', {
            ...
            email: resultdata.data[0].EMAIL,
            mobile: resultdata.data[0].PHONE_NUMBER,
            profileimage: resultdata.data[0].RESUME ? resultdata.data[0].RESUME : "",
            id: resultdata.data[0].EMPLOYEE_ID
        })
    })
}
...
exports.employeeProductById = (req, res) => {
    axios.get(`http://localhost:3001/product/${req.params.id}`)
        .then(result => {
            var empData = {};
            if (result.data.success) {
                empData = result.data.data[0];
                console.log(empData)
            }
            else {
                empData.product_id="-";
                empData.product_name="-";
                empData.price="-";
                empData.created_at="-"
            }
            res.render('products', {
                id: empData.product_id,
                name: empData.product_name,
                price: empData.price,
